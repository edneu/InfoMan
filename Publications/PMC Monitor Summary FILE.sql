...
ALTER TABLE work.PMCMon ADD PMCite varchar(4000);

SET SQL_SAFE_UPDATES = 0;



UPDATE work.PMCMon pm, work.pubmed_raw lu
SET pm.PMCID=lu.PMCID,
    pm.PMCite=lu.Citation
WHERE pm.PMID=lu.PMID;    



select * from work.PMCMon;

drop table if exists work.temp8;
create table work.temp8 AS
Select 	0 as OrigSeq, 
		"PMC Monitor" as Source,
        " " AS Article,
        PMID,
        PMCID,
        Article_Title AS RawCite,
        Status,
        "" AS Category,
		"" AS ForeComp,
        PMCite AS Pubmed_Cite
from work.PMCMon WHERE CTSI=1 AND PUBYEAR=2019;

Select count(distinct PMID) from work.PMCMon WHERE  CTSI=1 AND PUBYEAR=2019;
