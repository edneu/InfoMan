...
WHERE CLK_AWD_PROJ_ID IN (select distinct PRJNUM from space.ctrb_projects_2020)
...
select sr.CLK_AWD_PI,
       ...
       sp.ROOM  
from work.bondSRMRoom sr LEFT JOIN space.ctrb_projects_2020 sp ON sr.CLK_AWD_PROJ_ID=sp.PRJNUM ;
...
from work.bondSRMProj bsp LEFT JOIN space.ctrb_occs_2020 occ ON bsp.ROOM=occ.ROOM
...
WHERE Room_Occupant<>""
GROUP BY CLK_AWD_ID,
       CONCAT(Room_Occupant," (CTRB",ROOM,", ",OCCDESC ,", ",Room_Occupant_Dept,")") ;
...
select * from space.ctrb_occ_2019;
