...
select EMAIL from work.redsurveydata where EMAIL not in (select distinct EMAIL from space.bondmaster);

select distinct EMAIL from space.bondmaster where EMAIL not in (select distinct EMAIL from work.redsurveydata);

select EMAIL from work.bondmaster
...
#drop table if exists loaddata.backupbondmaster2018Survey;
#create table loaddata.backupbondmaster2018Survey as select * from space.bondmaster;
