...
select * from pilots.PILOTS_MASTER WHERE Award_year=2018;


################
###FIND IF FOR MISSING PILOTS

SELECT UF_UFID,UF_EMAIL,UF_FIRST_NM,UF_LAST_NM
FROM lookup.ufids
WHERE 
(UF_LAST_NM='Mobley' AND  UF_FIRST_NM='Amy ') OR
(UF_LAST_NM='Terada' AND  UF_FIRST_NM='Naohiro') OR
(UF_LAST_NM='Judge' AND  UF_FIRST_NM='Sarah') OR
(UF_LAST_NM='McQuail' AND  UF_FIRST_NM='Joseph') OR
(UF_LAST_NM='Fredenburg' AND  UF_FIRST_NM='Kristina') OR
(UF_LAST_NM='Gaynor' AND  UF_FIRST_NM='Leslie') OR
(UF_LAST_NM='Humes' AND  UF_FIRST_NM='Sara') OR
(UF_LAST_NM='Kissoon' AND  UF_FIRST_NM='Trisha') OR
(UF_LAST_NM='Weyh' AND  UF_FIRST_NM='Ashleigh') OR
(UF_LAST_NM='Ghasemi' AND  UF_FIRST_NM='Amir') OR
(UF_LAST_NM='Mahmud' AND  UF_FIRST_NM='Iqbal') ;
...
DROP TABLE if exists work.temp;
create table work.temp AS
SELECT Name,Employee_ID,Department_Code,Department,Salary_Plan,Job_Code,Faculty,FacType,FacultyType
...
DROP TABLE if exists work.temp;
create table work.temp AS
SELECT UF_UFID,UF_DISPLAY_NM,UF_GENDER_CD,UF_BIRTH_DT
FROM lookup.ufids
WHERE UF_UFID IN ('25384029');
('69621929',
'04975942',
'41398931',
'69393906',
