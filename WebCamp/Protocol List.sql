...
CREATE TABLE ctsi_webcamp_adhoc.Encounters AS
    SELECT VISITDATE AS EncounterDate,
      ...
      WHERE PROTOCOL IS NOT NULL 
       AND STATUS IN (2,3)
       AND Year(VISITDATE) IN (2015,2016,2017) 
     UNION ALL
    SELECT ADMITDATE AS EncounterDate,
      ...
      WHERE PROTOCOL IS NOT NULL 
       AND STATUS IN (2,3) 
       AND Year(ADMITDATE) IN (2015,2016,2017)
    UNION ALL
    SELECT ADMITDATE AS EncounterDate,
           PROTOCOL,
           PATIENT,
...
## FORMATTED TABLE
drop table if exists ctsi_webcamp_adhoc.ActiveProtocol20152017;
CREATE TABLE ctsi_webcamp_adhoc.ActiveProtocol20152017 AS
SELECT CRCNumber,
       PI_LastName,
	   PI_FirstName,
       Title,
...
select * from ctsi_webcamp_adhoc.ActiveProtocol20152017;
