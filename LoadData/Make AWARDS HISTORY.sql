...
load data local infile "P:\\My Documents\\My Documents\\LoadData\\AwardsHistory20210107.csv" 
...
UPDATE loaddata.awards_history ah SET Roster2020=1 Where Year(FUNDS_ACTIVATED)=2020 AND ah.CLK_PI_UFID IN (SELECT DISTINCT UFID from lookup.roster Where Year=2019 and UFID<>0 AND UFID IS NOT NULL);
...
SET sql_mode = '';
SET SQL_SAFE_UPDATES = 0;
create table loaddata.backupAwardsHistory2010107 AS SELECT * from lookup.awards_history;
